...
const tools: Tool[] = [
  ...
  },
  {
    ...
    desc: "Optimize your image file size for easier sharing.",
    icon: ImageIcon,
    implemented: true,
    path: "/image-compress",
    category: "Optimize",
  ...
  const handleAction = async (path: string, title: string) => {
    ...
    try {
      ...
      setPipelineFile({
        ...
        type:
          file.type ||
          (file.name.endsWith(".zip") ? "application/zip" : "application/pdf"),
      });
    ...
    <div className="fixed inset-0 z-[600] flex items-end sm:items-center justify-center p-0 sm:p-6 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="w-full max-w-md bg-[#FAFAFA] dark:bg-zinc-950 rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden border-t border-x border-white/10 sm:border animate-in slide-in-from-bottom-full duration-500 ease-out">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              {onBack && (
                <button
                  onClick={onBack}
                  className="p-2 -ml-2 text-gray-400 hover:text-rose-500 transition-colors"
                >
              ...
              <div className="min-w-0">
                <h3 className="text-lg font-black dark:text-white truncate max-w-[200px] leading-none mb-1">
                  {file.name}
                </h3>
            ...
            <button
              onClick={onClear}
              className="p-2 bg-gray-100 dark:bg-zinc-900 rounded-full text-gray-400 hover:text-rose-500 transition-colors"
            >
              <X size={18} />
            </button>
          </div>
        ...
        <div className="px-6 pb-6 space-y-4 max-h-[60vh] overflow-y-auto scrollbar-hide">
          <div>
            <h4 className="text-[10px] font-black text-gray-400 dark:text-zinc-500 uppercase tracking-[0.2em] mb-3 ml-1">
              Essentials
            </h4>
            <div className="grid grid-cols-2 gap-2.5">
              {essentials.map((tool) => (
                <button
                  key={tool.title}
                  onClick={() =>
                    tool.path && handleAction(tool.path, tool.title)
                  }
                ...
                >
                  <div
                    className={`p-2 rounded-xl ${tool.bg} ${tool.color} group-active:scale-110 transition-transform`}
                  >
...
function App() {
  ...
  useEffect(() => {
    ...
    const timerMinutes = parseInt(
      localStorage.getItem("autoWipeTimer") || "15",
    );
    ...
    if (isAutoWipeEnabled) {
      ...
      if (
        timerMinutes === 0 ||
        (lastSeen > 0 && elapsedMinutes >= timerMinutes)
      ) {
        clearActivity().then(() => {
          console.log(
            `Auto-Wipe triggered (${elapsedMinutes.toFixed(1)}m inactivity).`,
          );
        });
      }
    }
  ...
  useEffect(() => {
    const handleIntentFile = async (uri: string) => {
      try {
        ...
        const blob = await (
          await fetch(`data:application/pdf;base64,${fileContent.data}`)
        ).blob();
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("open-quick-drop" as any, handleGlobalTrigger);
  }, []);
        ...
        <PipelineProvider>
          ...
          >
            <Toaster
              ...
              toastOptions={{
                className:
                  "dark:bg-zinc-900 dark:text-white dark:border-white/10 mt-12",
                style: { zIndex: 1000 },
              }}
            />
            ...
            <Suspense fallback={<LoadingSpinner />}>
              <Routes>
                  ...
                  element={
                    viewMode === "web" ? (
                      ...
                      <AndroidView
                        ...
                        onFileSelect={(file) => handleGlobalDrop([file] as any)}
                      />
                    )
                  }
                />
                <Route
                  path="/android-tools"
                  element={<AndroidToolsView tools={activeTools} />}
                />
                <Route
                  path="/android-history"
                  element={<AndroidHistoryView />}
                />
                ...
                <Route path="/image-compress" element={<ImageCompressTool />} />
                <Route path="/grayscale" element={<GrayscaleTool />} />
                <Route path="/about" element={<About viewMode={viewMode} />} />
                <Route path="/privacy" element={<PrivacyPolicy />} />
                <Route
                  path="/settings"
                  element={<SettingsView theme={theme} setTheme={setTheme} />}
                />
